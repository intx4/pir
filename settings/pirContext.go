...
func NewPirContext(Items int, Size int, N int, Dimentions int) (*PirContext, error) {
	ctx := &PirContext{Items: Items, MaxBinSize: int(math.Floor((float64(TUsableBits*N) - math.Log2(float64(TUsableBits*N)) - float64(3*TUsableBits)) / float64(Size+8))), N: N, Dim: Dimentions} //- for padding and length, +8 is 1 byte for separator "|"
	ctx.Kd = int(math.Ceil(math.Pow(float64(Items)/float64(ctx.MaxBinSize), 1.0/float64(Dimentions))))
	ctx.K = int(math.Pow(float64(ctx.Kd), float64(Dimentions)))
	ctx.ExpBinSize = ctx.MaxBinSize
	return ctx, nil
}
