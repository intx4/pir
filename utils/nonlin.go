...
// EDIT: this does not panic on iteration limit
func (nk *NewtonKrylov) Solve(p nonlin.Problem, x []float64) nonlin.Result {
	var deriv nonlin.DerivativeApprox
	switch nk.Stencil {
	case 0, 4:
		deriv = nonlin.NewFourPoint(p.F, nk.StepSize)
	case 2:
		deriv = nonlin.NewCentral(p.F, nk.StepSize)
	case 6:
		deriv = nonlin.NewSixPoint(p.F, nk.StepSize)
	case 8:
		deriv = nonlin.NewEightPoint(p.F, nk.StepSize)
	default:
		deriv = nonlin.NewFourPoint(p.F, nk.StepSize)
	}
	deriv.X = x
	...
	for iter := 0; iter < nk.Maxiter; iter++ {
		...
		if err != nil {
			return nonlin.Result{X: []float64{}}
		}

		if nonlin.InfNorm(f0)+mat.Norm(res.X, math.Inf(1)) < nk.Tol {
			return nonlin.Result{
				X:         x,
				Converged: true,
				MaxF:      nonlin.InfNorm(f0),
				F:         f0,
			}
		}
